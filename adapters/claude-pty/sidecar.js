// claude-pty adapter — bus sidecar.
// Owns broker registration, inbound delivery socket, envelope render, and
// writing the rendered message into a PTY. Does NOT own the PTY lifecycle —
// the wrapper passes in a `write(text)` callback.
//
// The adapter renders each envelope to a sender-tagged, control-character-
// stripped string before writing it into the PTY, and speaks structured JSON
// only over its delivery socket.
...
// Delay between writing the rendered message and the submit-Enter; see
// handleRequest for why the TUI needs a tick in between.
...
// Resolve the adapter-sockets dir at call time, not module-load time. A
// module-level constant would freeze the env var at the FIRST require() of
// this file and ignore any test that tweaks the env after import. Caller may
// also pass an explicit `socketDir` option to bypass the env entirely.
...
function startSidecar({ write, sessionId, project, cwd, socketDir, getRegistrationProfile }) {
  ...
  // Minted fresh in start(); a same-uid peer that merely connects to the 0600
  // socket but does not present the token is rejected.
  ...
  async function handleRequest(req) {
    if (req.type === 'deliver' && req.envelope) {
      // Connection != authority. Only the broker (which received adapterToken at
      // register time) may drive the autosubmit-into-PTY path. A same-uid peer
      // that connected to the 0600 socket without the token is rejected here,
      // before any character is written to the live session.
      ...
      try {
        ...
        // Give the TUI a tick to absorb the typed characters into its input
        // buffer before submitting. Claude Code's TUI state machine reads
        // typed characters into its prompt, and if Enter arrives in the
        // same sync tick as the last char it sometimes lands before the
        // buffer is ready — the envelope renders in the input line but the
        // turn never submits.
        setTimeout(() => {
          try { write('\r'); } catch {}
        }, SUBMIT_DELAY_MS);
    ...
    if (req.type === 'send') {
      // Emitting as this participant is privileged: it sends under our broker
      // lease and is stamped with our identity. Require the capability token so
      // a same-uid peer cannot impersonate this session over its own socket.
      if (!sessionAuth.timingSafeEqualStr(req.auth, adapterToken)) {
        return { status: 'rejected', reason: 'unauthorized' };
      }
  ...
  async function start() {
    ...
    if (fs.existsSync(socketPath)) { try { fs.unlinkSync(socketPath); } catch {} }
    ...
    try {
      ...
      // Publish the capability token (0600) so the local CLI can read it for
      // {send}, and hand the same token to the broker via register so it can
      // present it on {deliver}. Written after the socket is up but before
      // register, so a delivery can never arrive before the token is on disk.
      ...
      await client.register({
        ...
        delivery: { adapter: 'claude-pty', socketPath, adapterToken },
      });
  ...
  return { start, stop, get socketPath() { return socketPath; }, get participantId() { return client ? client.participantId : null; } };
}
